class InkCodeHeader < ActiveRecord::Base
  # t.string   :name
  # t.string   :file_name
  # t.string   :run_type
  # t.string   :state
  ...
  def import_event
    ...
    if ink_code_creator.execute(run_type, DIRECTORY, file_name)
      logger.info "** InkCodeHeader #{id} import codes returned ok, marking complete"
      complete
    else
      logger.info "** InkCodeHeader #{id} import did NOT return ok, not marking complete"
    end
  end
