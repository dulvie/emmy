class AccountsReceivablesController < ApplicationController
  ...
  def index
    ...
    account = current_organization.accounts.where('accounting_plan_id = ? AND default_code_id = ?', @accounts_receivable.accounting_plan_id, @accounts_receivable.default_code_id).first
